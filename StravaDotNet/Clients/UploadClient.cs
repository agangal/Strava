...
namespace StravaDotNet
{
    ...
    public class UploadClient 
    {
        ...
        {
            string checkUrl = string.Format("{0}/{1}?access_token={2}", Endpoints.Uploads, uploadId, AuthSettings.ScopeAccessToken);
            string json = await HttpHelper.GetRequestAsync(checkUrl);
        ...
        {
            string checkUrl = string.Format("{0}/{1}?access_token={2}", Endpoints.Uploads, uploadId, AuthSettings.ScopeAccessToken);
            string json = HttpHelper.GetRequest(checkUrl);
